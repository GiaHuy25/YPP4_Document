...
BEGIN
    ...
    WHERE EntityId IN (SELECT FileId FROM inserted)
    AND EntityTypeId = 1; -- File

    -- Insert new SearchIndex rows with tokenized data from name and content
    WITH Tokenized AS (
        SELECT 
            i.FileId AS EntityId,
            1 AS EntityTypeId, -- File
    ...
    INSERT INTO SearchIndex (EntityId, EntityTypeId, Term, TermFrequency, DocumentLength, TermPositions)
    SELECT EntityId, EntityTypeId, Term, TermFrequency, DocumentLength, TermPositions
    FROM Tokenized;
END;
GO
...
BEGIN
    ...
    DELETE FROM SearchIndex
    WHERE EntityId IN (SELECT FolderId FROM inserted)
    AND EntityTypeId = 2; -- Folder

    -- Insert new SearchIndex rows with tokenized data
    WITH Tokenized AS (
        SELECT 
            i.FolderId AS EntityId,
            2 AS EntityTypeId, -- Folder
            LOWER(t.value) AS Term, -- Case-insensitive terms
            COUNT(*) AS TermFrequency,
            STRING_AGG(CAST(t.rn AS NVARCHAR), ',') AS TermPositions,
            t.DocLength AS DocumentLength
