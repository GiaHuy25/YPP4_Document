...
VALUES
  ...
  ----1----
  SELECT COUNT(DISTINCT customer_id) AS COUNT_CUSTOMER_UNIQUE
  FROM subscriptions
  ----2----
  ...
  GROUP BY DATEPART(MONTH,subscriptions."start_date")
  ORDER BY "MONTH"
  ----3----
  SELECT plans.plan_id,
		 plans.plan_name,
		 COUNT( subscriptions.customer_id) AS COUNT_CUSTOMER
  FROM subscriptions
  JOIN plans
  ON subscriptions.plan_id = plans.plan_id
...
SELECT 
	...
	ROUND(100.0 * COUNT(
						CASE 
						WHEN row_num = 2 AND plan_name = 'churn' THEN 1 ELSE 0 END) / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions),1
	) AS churn_percentage
...
FROM (SELECT customer_id,
	 ...
	 FROM subscriptions
     WHERE start_date <= '2020-12-31') as  next_dates
